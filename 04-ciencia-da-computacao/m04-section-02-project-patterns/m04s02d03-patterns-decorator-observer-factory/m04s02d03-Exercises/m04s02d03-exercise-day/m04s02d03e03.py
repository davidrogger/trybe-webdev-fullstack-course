# Exercício 3: Você recebeu um convite para ajudar com uma
# atualização de segurança em um sistema interno de uma empresa.
# Esse sistema atende os setores de vendas, financeiro e suporte.
# Para respeitar a nova Lei Geral de Proteção de Dados, a empresa
# precisa garantir que as informações internas serão expostas somente
# a quem tem necessidade. Assim devemos ter os seguintes perfis de acesso:
...
class FinanceSystemAcess(SystemAcess):
    def __init__(self, can_access=False):
        self.name = "Finance"
        self.set_permission(can_access)
...
class SalesSystemAcess(SystemAcess):
    def __init__(self, can_access=False):
        self.name = "Sales"
        self.set_permission(can_access)
...
class ManagerAccount(Account):
    def create_account(self):
        self.add_permissions(SupportAccount(True))
        self.add_permissions(FinanceSystemAcess(True))
        self.add_permissions(SalesSystemAcess(True))
